% Preprocessing pipeline for ECoG data of Patient-2 (no triggers this time)
%
...
for run_i = 1:n_runs
    for seg_i = 1:n_segs
        ...
        disp(['run ' runID(run_i) ' segmenting ' seg_type{seg_i}])
    %--------------------- Load the dataset ------------------------------------
 
    if filtered
        if baseline
            output_file = sprintf('run_%s_seg_%s_filter_baseline.mat', runID(run_i), seg_type{seg_i});
        else
            output_file = sprintf('run_%s_seg_%s_filter_nobaseline.mat', runID(run_i), seg_type{seg_i});
        end
        data_dir = '/home/rajat/Dropbox/JapanECoG/raw_data/Patient2/';
        data_file = fullfile(data_dir,['action_movie_' runID(run_i) '.mat']);
        load(data_file) % loads raw (channels X timepoints) and hdr (header info)
        hdr.label(63)=[]; % triggers at movie begin and dummy trigger channel
        raw(63,:)=[]; % delete them from the data as well.

    else
        if baseline
            output_file = sprintf('run_%s_seg_%s_nofilter_baseline.mat', runID(run_i), seg_type{seg_i});
        else
            output_file = sprintf('run_%s_seg_%s_nofilter_nobaseline.mat', runID(run_i), seg_type{seg_i});
        end
        data_dir = '/home/rajat/Dropbox/JapanECoG/raw_data/Patient2/data_nofilter';
        % trigger channel 63 from the filtered case is not present.
        data_file = fullfile(data_dir,['action_movie_' runID(run_i) '.mat']);
        load(data_file) % loads raw (channels X timepoints) and hdr (header info)
        raw(125,:) = raw(124,:); % create a dummy channel for math to work in what follows
        hdr.label{125} = 'dummy';
    end
    ...
    to_remove = [4,8,12,17,22,27,32,37,42,47,52,57,62]; % For all A channels
    to_remove = [to_remove, to_remove + 62]; % for B channels 
    ...
    if (strcmp(seg_type{seg_i}, 'camera'))
        ...
        cfg.trialdef.pre = 5.0; % 5 secs before movie starts 
